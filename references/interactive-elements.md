...
For the Codex `codebase-to-course` skill, all of these patterns should be delivered as inline HTML, CSS, and JavaScript inside one self-contained course file. Prefer progressively enhanced interactions so the page still reads well even if a script fails.
