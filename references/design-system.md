...
For the Codex `codebase-to-course` skill, keep these styles inline inside the generated HTML file. Do not split CSS into external assets; Google Fonts is the only allowed external dependency.
...
- Prefer CSS shapes, inline SVG, gradients, and shadows over external decorative image assets so the course stays self-contained
